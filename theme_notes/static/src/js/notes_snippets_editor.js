(function () {
    ...
    website.Theme.include({
        start: function () {
            this._super();
            this.$el.find('label:has(input[data-xmlid="theme_notes.option_bg_custom"])').on('mouseup', _.bind(this.set_custom_bg, this));
        },
        change_selection: function (event, force) {
            var self = this;
            if (!force && (event.originalEvent && $(event.target).is('input[data-xmlid="theme_notes.option_bg_custom"]'))) {
            ...
            this.set_custom_bg_time = setTimeout(function () {
                ...
                $image.on('saved', this, function () {
                    ...
                    openerp.jsonRpc('/web/dataset/call', 'call', {
                        ...
                        args: ["theme_notes", "option_bg_custom"],
                    }).then(function (data) {
