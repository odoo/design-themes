...
It is the eCommerce sibling of ``theme_preview_generator``, sharing its rewriting
into a palette-tokenized, JS-free page and its fleet of workers: keep both in
sync. On top of that it marks what the configurator personalizes, all optional
and falling back to the fake catalog baked in here:

- ``industry_image_key`` on an image, ``industry_text_key="<slot>.label"`` (or
  ``.price``) on a name or price, both naming an IAP slot;
...
WEBSITE_SALE_DIR = ADDONS_DIR / "website_sale"
LOGIN = "admin"
PASSWORD = "admin"
...
# Only needed to build the base database. Without this the demo catalog would
# show up next to the fake one.
INSTALL_ARGS = ["--init=website_sale", "--without-demo=True"]
...
#
# Every entry names its industry picture. A `cat_prod_` picture is the one of
# both a category and a product, which need different labels, so the catalog
# slot holding the label is a separate name (`CATALOG_ITEM_NAMES` in
# api_website) derived from the position below.
CATEGORY_DATA = [
    # name, static image, industry image
    ("Desks", "desks.jpg", "website.cat_prod_sq_1"),
    ("Cabinets", "cabinets.jpg", "website.cat_prod_sq_2"),
    ("Drawers", "drawers.jpg", "website.cat_prod_sq_3"),
    ("Lamps", "lamps.jpg", "website.cat_prod_sq_4"),
    ("Storage", "boxes.jpg", "website.cat_prod_sq_5"),
    ("Seating", "furnitures.jpg", "website.cat_prod_sq_6"),
    ("Bins", "bins.jpg", "website.cat_prod_sq_7"),
    ("Desk Parts", "desk_components.jpg", "website.cat_prod_sq_8"),
]
PRODUCT_DATA = [
    # name, price, description, static image, industry image
    ("Corner Desk", 420.00, "Solid oak top on a powder-coated steel frame.",
     "product_product_5-image_02.jpg", "website.cat_prod_sq_1"),
    ("Desk Lamp", 39.00, "Articulated arm with a warm dimmable light.",
     "product_lamp_02.jpg", "website.cat_prod_sq_2"),
    ("Office Chair", 189.00, "Adjustable headrest and breathable backrest.",
     "product_product_16-image_02.jpg", "website.cat_prod_sq_3"),
    ("Storage Cabinet", 245.00, "Two doors, three shelves, soft-close hinges.",
     "product_product_12-image_02.jpg", "website.cat_prod_sq_4"),
    ("Filing Pedestal", 159.00, "Three drawers on casters, fits under any desk.",
     "product_product_27-image_02.jpg", "website.cat_prod_sq_5"),
    ("Writing Desk", 340.00, "Compact desk with two hidden drawers.",
     "table02_02.jpg", "website.cat_prod_sq_6"),
    ("Work Pod", 890.00, "Acoustic shell for focused work in open spaces.",
     "product_product_24-image_02.jpg", "website.cat_prod_sq_7"),
    ("Drawer Unit", 199.00, "Oak front on a matte black steel body.",
     "product_product_10-image_02.jpg", "website.cat_prod_sq_8"),
    ("Desk Organizer", 12.50, "Perforated steel holder for pens and tools.",
     "desk_organizer_02.jpg", "website.prod_sq_1"),
    ("Floor Mat", 65.00, "Protects hard floors from chair casters.",
     "floor_protection-image_02.jpg", "website.prod_sq_2"),
]

# The product whose page is previewed shows several angles of itself. A square
# page layout fits four of them, a landscape one three, and the pictures differ,
# so the set is written per style on its own database copy.
PRODUCT_PAGE_SQUARE_IMAGES = [
    # static image, industry image
    ("product_product_5-image_02.jpg", "website.cat_prod_sq_1"),
    ("product_product_3-image_02.jpg", "website.cat_prod_sq_1_bis"),
    ("product_product_6-image_02.jpg", "website.cat_prod_sq_1_ter"),
    ("product_product_7-image_02.jpg", "website.cat_prod_sq_1_quater"),
]
...
# The styles whose `product_page_image_ratio` is 16/9 or 21/9. The others are
# square or portrait, close enough to crop a square picture.
LANDSCAPE_PRODUCT_PAGE_OPTIONS = ("large_grid", "large_image")


def build_images(directory, names):
    return [
        {"image": f"{directory}/{filename}", "image_key": image_key}
        for filename, image_key in names
    ]
...
CATEGORIES = [
    {
        ...
        "image_key": image_key,
        "label_key": f"website.shop_category_{index}",
    }
    for index, (name, filename, image_key) in enumerate(CATEGORY_DATA, start=1)
]
...
PRODUCTS = [
    {
        ...
        "images": build_images(PRODUCTS_IMG_DIR, [(filename, image_key)]),
        "label_key": f"website.shop_product_{index}",
    }
    for index, (name, price, description, filename, image_key) in enumerate(PRODUCT_DATA, start=1)
]
...
def get_product_page_images(option):
    names = (
        PRODUCT_PAGE_LANDSCAPE_IMAGES
        if option in LANDSCAPE_PRODUCT_PAGE_OPTIONS
        else PRODUCT_PAGE_SQUARE_IMAGES
    )
    return build_images(PRODUCTS_IMG_DIR, names)


def get_preview_texts():
    texts = {category["name"]: f"{category['label_key']}.label" for category in CATEGORIES}
    for product in PRODUCTS:
        texts[product["name"]] = f"{product['label_key']}.label"
        texts[f"{product['price']:.2f}"] = f"{product['label_key']}.price"
    return texts


PREVIEW_TEXTS = get_preview_texts()
...
def palette_color_expression(color, alpha=None):
    if alpha is None:
        return color
...
def prepare_base_database():
    ...
    server = start_odoo(BASE_DATABASE, *INSTALL_ARGS)
    ...
    product_ids = call_kw(session, context, "product.template", "create", [[
        {
            ...
            "image_1920": encode_image(product["images"][0]["image"]),
        }
...
def stamp_texts(soup):
    ...
    drop_leaked_texts(soup)


def drop_leaked_texts(soup):
...
def stamp_category_images(soup):
    ...
    for item in soup.select(".o_wsale_filmstrip_item"):
        category = categories.get(item.get_text(strip=True))
        ...
        image[IMAGE_KEY_ATTRIBUTE] = category["image_key"]
...
def stamp_shop_page(soup):
    stamp_logo(soup)
    stamp_texts(soup)
    stamp_category_images(soup)
    ...
    for tile in soup.select(".oe_product"):
        ...
        if product:
            stamp_record_images(tile, product["images"])


def stamp_product_page(soup, images):
    stamp_logo(soup)
    stamp_texts(soup)
...
def generate_style_preview(database, style_keyword, option, output_path, product_id):
    ...
    try:
        ...
        if is_shop_page:
            path, stamp = "/shop", stamp_shop_page
        else:
            ...
            def stamp(soup):
                stamp_product_page(soup, page_images)
