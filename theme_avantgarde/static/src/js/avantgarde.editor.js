(function () {
    ...
    website.snippet.options.background.include({
        select_class: function (type, value, $li) {
            ...
            if (this.$target.hasClass("s_media_block")) {
                var objs = this.$target.find(".videoBox, > iframe, .yt_video_container");
                objs.stop(true,true)
                    .fadeOut(100)
                    .delay(600)
                    .o_anim_fade_in(2000);
    ...
    website.snippet.options.avantgarde_top_banner_options = website.snippet.Option.extend({
        on_focus: function () {
            if (this.$target.hasClass("move_to_top")) {
                this.$overlay
        ...
        select_class: function (type, value, $li) {
            ...
            if (value == "move_to_top") {
                ...
                this.$target.addClass("move_to_top");
                $("#wrapwrap").addClass("top_content");
                $("#wrap").css("margin-top", (this.$target.height()* 0.9));
            ...
            if (this.$target.hasClass("s_media_block")) {
                ...
                this.$target.data('snippet-view').start_video();
            }
        },
    });
