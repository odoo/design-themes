(function(){
      ...
      $win
        ...
        .on("resize", function(){
          ...
          if($("#oe_main_menu_navbar").length > 0) { 
            ...
            $graph_header.find(".navbar").animate({top:(edit_bar_h *0.5)},100);
            if(graph_header_affix == true){$graph_header_clone.css("top", edit_bar_h)}
          }
          if(graph_top_content == true){
  ...
  website.snippet.animationRegistry.s_animated_boxes_slide = website.snippet.Animation.extend({
    ...
    start: function() {
      ...
      self.$target.on("mouseenter", function(){
        self.handlerIn(self.$target)
      });
      self.$target.on("mouseleave", function(){
        self.handlerOut(self.$target)
      })
    },
    ...
    handlerIn: function($el) {
      var $s = $el.find(".slide");
      $s.css("display", "block")
        .addClass($s.attr("graph_anim"));
    },

    handlerOut: function($el) {
      var $s = $el.find(".slide");
      $s.fadeOut(300, function() {
        $s.removeClass($s.attr("graph_anim"));
      });
    }
  })
