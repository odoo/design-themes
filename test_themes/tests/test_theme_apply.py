...
class TestThemeApply(TransactionCase):
    ...
    def test_addon_generation_covers_installed_themes(self):
        ...
        if 'website_sale' not in IrModule._installed():
            self.skipTest("website_sale is not installed.")
        addon_name = 'website_sale'
        # Declared by theme Nano in its `configurator_snippets_addons`.
        key = 'website_sale.configurator_homepage_s_dynamic_snippet_category_list'
